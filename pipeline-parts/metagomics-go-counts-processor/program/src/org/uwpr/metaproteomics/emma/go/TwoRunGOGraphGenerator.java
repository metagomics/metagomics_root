...
import org.uwpr.metagomics.go_counter.program.TwoRunGraphOb;
...
public class TwoRunGOGraphGenerator {

	private String getLabel( TwoRunGraphOb ob ) {
		
		DecimalFormat df = new DecimalFormat("####0.##");
		
		String label = ob.getNode().getName() + " (" + ob.getNode().getAcc() + ")";
		
		label += "\n" + df.format( ob.getLaplaceLogChange());
		label += "\n" + String.format( "%3.2E", ob.getLaplaceQvalue() );
		
		return label;
	}
	
	
	public void saveGOGraphSVGImage( Map<String, TwoRunGraphOb> data, File file ) throws Exception {
			...
			if( ob.getParents() != null && ob.getParents().size() >= 1 ) {
				...
				if (parents != null) {
					...
					while (piter.hasNext()) {
						...
						TwoRunGraphOb gparent = data.get( parentNode.getAcc() );
						Node parent = null;
	...
	public BufferedImage getGOGraphImage( Map<String, TwoRunGraphOb> data ) throws Exception {
		Graph2D graph = new Graph2D();
		Map<String, Node> addedNodes = new HashMap<String, Node>();
	...
	private Color getColor( TwoRunGraphOb ob ) {
		int base = 210;
		...
		int modifier = getModifier( ob );
	
		if( ob.getLaplaceLogChange() >= 0 ) {
			blue -= modifier;
		} else {
			red -= modifier;
			green -= modifier;
	...
	private int getModifier( TwoRunGraphOb ob ) {
		
		int log = Math.abs( (int)(Math.log10( ob.getLaplaceQvalue() ) ) );
	...
	private void modifyRealize( ShapeNodeRealizer realizer, TwoRunGraphOb ob ) {
		
		realizer.setFillColor( this.getColor( ob ) );
		

		if( ob.getLaplaceLogChange() > 0 ) {
			realizer.setShapeType( ShapeNodeRealizer.PARALLELOGRAM );
		} else {
			realizer.setShapeType( ShapeNodeRealizer.ROUND_RECT );
		}
