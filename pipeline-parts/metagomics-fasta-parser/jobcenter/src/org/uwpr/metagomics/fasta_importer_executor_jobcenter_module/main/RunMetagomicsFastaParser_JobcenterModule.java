...
import org.uwpr.metagomics.fasta_importer_executor_jobcenter_module.config.GetConfigFromFileServiceImpl;

/**
 * main class executed by  Client
 *
 */
public class RunMetagomicsFastaParser_JobcenterModule implements ModuleInterfaceClientMainInterface {

	private static Logger log = Logger.getLogger(ModuleInterfaceClientMainInterface.class);


	private static final String JOB_PARAM_FASTA_FILE_ID = "fasta_file_id";

	private static final String MODULE_LABEL = "Metagomics_FASTA_Importer_Module";
...
//	Job Manager lifecycle methods
	...
	public void processRequest( ModuleInterfaceRequest moduleInterfaceRequest, ModuleInterfaceResponse moduleInterfaceResponse, ModuleInterfaceJobProgress moduleInterfaceJobProgress, ModuleInterfaceClientServices moduleInterfaceClientServices  ) throws Throwable {
		...
		String fastaFileId = jobParameters.get( JOB_PARAM_FASTA_FILE_ID );
		...
		try {
			...
			if ( StringUtils.isEmpty( fastaFileId ) ) {

				String msg = methodName + ": job parameter " + JOB_PARAM_FASTA_FILE_ID + " cannot be empty or null.";
...
//				Integer jobRequiredExecutionThreads = moduleInterfaceRequest.getJobRequiredExecutionThreads();
				...
				log.info( "Parameters from job:  '" + JOB_PARAM_FASTA_FILE_ID + "' "
						+ " = '" + fastaFileId + "'" );
			}
			...
			boolean runResult = 
					RunMetagomicsFastaParser_Main
					.getInstance().runCommandForParams( fastaFileId, commandToExecuteBase );
			...
			if ( runResult ) {
				...
				msg = "Successful completion of fastaFileId: " + fastaFileId;
