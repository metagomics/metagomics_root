package org.uwpr.metagomics.run_blast.run_system_command;
...
import org.uwpr.metagomics.run_blast.exceptions.MetagomicsConfigException;
...
//			exittedWaitDueToAwakenNotify = true;
			...
			boolean throwExceptionOnCommandFailure ) throws Exception
	{
		...
		if ( log.isInfoEnabled() ) {
			System.out.println( "runCmd: running shell command:  " + commandAndItsArgumentsList );
		}
		...
		try {
			...
			if ( exitValue != 0 ) {
				...
				if ( log.isInfoEnabled() ) {
					System.out.println( "runCmd: shell command SUCCESSFUL:  " + commandAndItsArgumentsList );
				}
			}
			if ( shutdownRequested ) {
				//  Shutdown Requested so early exit
			...
			log.error( "Exception running command |" + commandAndItsArgumentsList + "|.  Exception: " + e.toString(), e );
			throw e;
		} finally {
